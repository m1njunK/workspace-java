...
public class BookManagement {
	...
	public void  registerBook() {
		...
		String autor = getData("등록할 책의 저자를 입력하세요 > ");
		
		Book book = new Book(count,title,autor);
	...
	public void updateBook() {
		...
		findBook = findBook(bookNum);

		if(findBook == null) {
			System.out.println("찾으시는 번호의 책이 없슴다.");
			return;
		}
		...
		int selectNum = getSelectNum("번호입력 >");
			...
			switch(selectNum) {
				...
				case 1 :
					...
					System.out.println("제목수정 완료");
					break;
					
				case 2 :
					System.out.println("저자수정");
					String newAutor = getData("저자입력 > ");
					findBook.setAuthor(newAutor);
					break;
	...
	public void deleteBook() {
		...
		removeBook = findBook(bookNum);
		
		if(removeBook == null) {
			System.out.println("찾으시는 책이 없슴다.");
		}
	...
	public Book findBook(int num) {
		...
		for(Book b : books) {
			if(b != null && b.getNum() == findBook.getNum()) {
				return b;
			}
		}
		return null;
